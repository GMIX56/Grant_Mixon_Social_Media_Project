...
class Profile(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE) # associating Profile with User model
    friends = models.ManyToManyField(User,blank=True, related_name='friends') # user can have many friends
    created = models.DateTimeField(auto_now=True) # date time when profile is created
    updated = models.DateTimeField(auto_now_add=True) # date time when profile is updated
...
class Post(models.Model): 
    ...
    image = models.ImageField(upload_to='images',blank=True) # will not work if no pillow installed
...
class Comment(models.Model):
    post = models.ForeignKey(Post, on_delete=models.CASCADE) # what post the comment is associated with
    username = models.ForeignKey(User, related_name='details', on_delete=models.CASCADE) # who is commenting on the post
    text = models.CharField(max_length=200) # what it says
